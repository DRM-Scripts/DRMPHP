...
function LoadMPD($mpd_url, $UseProxy, $Proxy, $Useragent){
  $data=Download($mpd_url, $UseProxy, $Proxy, $Useragent);
  if($data){
...
function derive_segment_URLs($mpd_features, $current_period, $urls, $period_info){
  ...
  foreach($adaptation_sets as $i => $adaptation_set){
    if($adaptation_set["ProducerReferenceTime"][0]["wallClockTime"]!=""){
      $WallClock=$adaptation_set["ProducerReferenceTime"][0]["wallClockTime"];
      $TimeDelta = time() - strtotime($WallClock);
    }
    ...
    case 'SegmentTemplate':
      ...
      if($segment_timeline != NULL){
        ...
        if($S_array != NULL){
          ...
          //echo "segment time: ".$segment_time."\r\n";
          $segment_time -= $pto;
          //echo "segment time: ".$segment_time."\r\n";
          $segment_time -= $availabilityTimeOffset;
          //echo "segment time: ".$segment_time."\r\n";
          if(intval($TimeDelta) > 0){
            $segment_time -= (10 * 60) * $timescale;
            //echo "segment time: ".$segment_time."\r\n";
          }
          //echo "--------------------------------\r\n";
...
function UpdatePlaylist($ChName, $ChID, $map2){
  ...
  for($i=0;$i<count($Files);$i++){
    ...
    if($seq==0)$seq=intval(str_replace($OutExt,"",$file));
    $cmd=$FFMpegBin." -hide_banner -i ".$Files[$i]." > tmp.txt 2>&1";
    exec($cmd);
    $tmp=file("tmp.txt");unlink("tmp.txt");
    foreach($tmp as $line){
    ...
    $dur=$dur[count($dur)-1];
    if($dur > $MaxDur)$MaxDur = number_format($dur,2,'.','');
...
function Download($url, $UseProxy=0, $Proxy=[], $Useragent=""){
  $ch = @curl_init();
...
function DownloadList($List, $UseProxy=0, $Proxy=[]){
...
try {
  ...
  if(intval($UpdateInterval) == 0)$UpdateInterval=array_sum($MyD)/count($MyD);
  if(intval($UpdateInterval) == 0)$UpdateInterval=2.1;
  ...
  switch($RunMode){
    case "infoshort":{
      ...
      foreach($ARep as $ar){
        foreach($VRep as $vr){
          if(!$vr["width"])$vr["width"]=$vr["maxwidth"];
          if(!$vr["height"])$vr["height"]=$vr["maxheight"];
          if(!$vr["frameRate"])$vr["frameRate"]=$vr["aframerate"];
    ...
    case "infolong":{
      foreach($adaptation_sets as $Adpt){
        $Reprs = $Adpt['Representation'];  
        foreach($Reprs as $Rep){
          if($Adpt["mimeType"]=="audio/mp4") $ARep[]=$Rep;
          if($Adpt["mimeType"]=="video/mp4") $VRep[]=$Rep;
        }
      }
      echo "Audio:\r\n";
      foreach($ARep as $r){
    ...
    case "testonly":{
      foreach($adaptation_sets as $Adpt){
        $Reprs = $Adpt['Representation'];  
        foreach($Reprs as $Rep){
          if($Adpt["mimeType"]=="audio/mp4") {$Rep["lang"]=$Adpt["lang"];$ARep[]=$Rep;}
          if($Adpt["mimeType"]=="video/mp4") $VRep[]=$Rep;
        }
      }
      echo "Audio:\r\n";
    ...
    case "download":{
      ...
      for($k=0;$k<count($a_url);$k++){
        array_shift($a_url[$k]); array_shift($a_url[$k]);
      }
      array_shift($v_url); array_shift($v_url);
      ...
      for($i=0;$i<count($Timeline);$i++){
        if($Timeline[$i]["s"]==0)$ToDownload=$ToDownload+1;
      }
