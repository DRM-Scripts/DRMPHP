...
<div class="input-append">
  <select <?=$Disabled?> id="Variant">
    ...
    ?>
      <option <?=$Selected?> value="<?=($AudioID."|".$VideoID)?>">L: <?=$V["Language"]?>, A: <?=$V["AudioID"]." ".$V["AudioBandwidth"]?>, V: <?=$V["VideoID"]." ".$V["VideoBandwidth"]?></option>
    <?
    }
    ?>
  </select>
  <a <?=$Disabled1?> style="display:<?=$Display1?>" class="btn btn-success btn-sm " href="javascript: void(0)" onclick="save()">save</a>
  <a <?=$Disabled2?> style="display:<?=$Display2?>" class="btn btn-warning btn-sm " href="javascript: void(0)" onclick="Parse()">Parse</a>
...
<script>
  function Parse(){
    var chanid='<?=$ID?>';
    $.post("_func.php",{Func:"Parse", ID:chanid})
    .done(function(data){
      $('#Config_'+chanid).load("_chanconfig.php",{ChanID:chanid});
    });
  }
  function save(){
    $.post("_func.php", {Func:"SaveVariant", Variant:$('#Variant').val(), ChanID:'<?=$ID?>'})
    .done(function(){
